...
require('fpdf/fpdf.php');
	...
	$manufacture = (($actype == "A330-200") || ($actype == "A330-300") ? "AIRBUS" : (($actype == "CRJ-1000") ? "BOMBARDIER" : "BOEING"))." ".((strpos($actype,",")) ? "B737-Classic" : $actype);
...
$panjang = $tinggi*54-$tinggi/2;
	...
	if(isset($FHlastyear)){
		$RFlastyear = ((((120 + (((int)$FClastyear - 1)*60)) + 80) * 1.1)/((int)$FHlastyear * 60));
	}
	
	$pdf->Cell($L,$tinggi,(isset($FHlastyear)) ? (round(array_sum($FHlastyear) * $RFlastyear,0)) : 0,'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		if (!empty($FH[$value])){
			$RF = ((((120 + (((int)$FC[$value] - 1)*60)) + 80) * 1.1)/((int)$FH[$value] * 60));
		}
		
		$pdf->Cell($L,$tinggi,($FH[$value]) ? (round($FH[$value] * 1,0)) : 0,'TBLR',0,'C');
	}
...
$getRY = $pdf->GetY();
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, (isset($FHlastyear)) ? round(($engineremovallastyear["Non Basic UnPlanned"] + $engineremovallastyear["Basic UnPlanned"])/(array_sum($FHlastyear) * $RFlastyear)*1000,3) : "",'TBLR',0,'C');
	//die(print_r($FH24));
foreach($bulan as $key => $value)
	{
		if (!empty($FH[$value])){
		...
		$pdf->Cell($L,$tinggi, (!empty($FH[$value]) ? round((($engineremoval["Basic UnPlanned"][$value] + $engineremoval["NonBasic UnPlanned"][$value])/($FH[$value] * $RF)*1000),3) : 0),'TBLR',0,'C');

		$uer[] =  !empty($FH[$value]) ? round((($engineremoval["Basic UnPlanned"][$value] + $engineremoval["NonBasic UnPlanned"][$value])/($FH[$value] * $RF)*1000),3) : 0;
	}
	...
	foreach($bulan as $key => $value)
	{
		if (!empty($FH24[$bulan12[$key+10]])){
			$RF3months = ((((120 + ((($FC24[$bulan12[$key+10]] + $FC24[$bulan12[$key+11]] + $FC24[$bulan12[$key+12]]) - 1)*60)) + 80) * 1.1)/(($FH24[$bulan12[$key+10]] + $FH24[$bulan12[$key+11]] + $FH24[$bulan12[$key+12]]) * 60));
		}
		
		$pdf->Cell($L,$tinggi, (!empty($FH24[$bulan12[$key+10]]) ? round((($engineremoval["Basic UnPlanned"][$bulan12[$key+10]] + $engineremoval["Non Basic UnPlanned"][$bulan12[$key+10]] + $engineremoval["Basic UnPlanned"][$bulan12[$key+11]] + $engineremoval["Non Basic UnPlanned"][$bulan12[$key+11]] + $engineremoval["Basic UnPlanned"][$bulan12[$key+12]] + $engineremoval["Non Basic UnPlanned"][$bulan12[$key+12]])/(($FH24[$bulan12[$key+10]] + $FH24[$bulan12[$key+11]] + $FH24[$bulan12[$key+12]]) * $RF3months)*1000),3) : 0),'TBLR',0,'C');
	}
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, (isset($FHlastyear)) ? round((($SVlastdata)/(array_sum($FHlastyear)*$RFlastyear)*1000),3) : "",'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		if(!empty($FH[$value])){
			$RF = ((((120 + (($FC[$value] - 1)*60)) + 80) * 1.1)/($FH[$value] * 60));
		}
		$shopvisitrate[$value] = !empty($FH[$value]) ? round((($SV[$value])/($FH[$value] * $RF)*1000),3) : 0;
	...
	foreach($bulan as $key => $value)
	{
		...
		$pdf->Cell($L,$tinggi, (!empty($FH24[$bulan12[$key+10]]) ? round((($SV[$bulan12[$key+10]] + $SV[$bulan12[$key+11]] +  $SV[$bulan12[$key+12]])/(($FH24[$bulan12[$key+10]] + $FH24[$bulan12[$key+11]] + $FH24[$bulan12[$key+12]]) * $RF3months)*1000),3) : 0),'TBLR',0,'C');
	}
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, (isset($FHlastyear)) ? round((($engineremovallastyear["Basic Planned"] + $engineremovallastyear["Basic UnPlanned"] + $engineremovallastyear["Non Basic Planned"] + $engineremovallastyear["Non Basic UnPlanned"])/(array_sum($FHlastyear)* $RFlastyear)*1000),3) : "",'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		if(!empty($FH[$value])){
			$RF = ((((120 + (($FC[$value] - 1)*60)) + 80) * 1.1)/($FH[$value] * 60));
		...
		$engremovalrate[$value] = !empty($FH[$value]) ? round((($engineremoval["Basic Planned"][$value] + $engineremoval["Basic UnPlanned"][$value] + $engineremoval["Non Basic Planned"][$value] + $engineremoval["Non Basic UnPlanned"][$value])/($FH[$value] * $RF)*1000),3) : 0;
	...
	for($i=1;$i<13;$i++)
	{
		...
		$FCengremove += $FC24[$bulan12[$key+$i]];
	}
	
	if(!empty($FHengremove)){
		$RF12months = ((((120 + (($FCengremove - 1)*60)) + 80) * 1.1)/($FHengremove * 60));
	}
	
	$rate = !empty($FHengremove) ? round(($predbasplan + $predbasunplan + $prednonbasplan + $prednonbaseunplan)/($FHengremove * $RF12months)*1000,3) : 0;
