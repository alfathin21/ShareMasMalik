...
<body>
 ...
 <form name="frm1">
 <p>Periode (yyyy-mm) &nbsp&nbsp&nbsp<?php
...
<?php 
	...
	$aircrafttype = array("A330-200","A330-300","B737-300","B737-400","B737-500"/*,"B737-Classic"*/,"B737-800-E","B737-800-M","B747-400","CRJ-1000","A320-200","B737-CL","B777-300","ATR72-600");
	$acclassic = array("B737-300","B737-400","B737-500");
	
	foreach($aircrafttype as $k => $v){
		
		/*foreach ($acclassic as $key => $value){
		
			$delayCL += $delay[$value];
			$FHCL += $FH[$value];
			$cancelCL += $cancel[$value];
			$FCrevCL += $FCrev[$value];
			$delayallCL = $delayallCL[$value];
		
			$ifsdCL += $ifsd[$value];
			$availableCL += $available[$value];
		}*/
...
?>
   <tr>
     <th scope="row"><span class="style2">B737-Classic</span></th>
	<?php
		$last12moCL = 0;
		foreach ($bulan as $key => $value){
			$totaldelayclassic = 0;
			foreach ($acclassic as $k => $v){
				$totaldelayclassic += $delay[$v][$value];
				$last12moCL += $delay[$v][$value];
			}
	?>
     <td><div align="center" class="style2"><?= $totaldelayclassic?></div></td>
	<?php
		}
	?>
	<td><div align="center" class="style2"><?= $last12moCL?></div></td>
   </tr>
   <tr>
...
?>
   <tr>
	...
	<?php
		foreach ($bulan as $key => $value){
			$totalcancelclassic = 0;
			foreach ($acclassic as $k => $v){
				$totalcancelclassic += $cancel[$v][$value];
			}
	?>
     <td><div align="center" class="style2"><?= $totalcancelclassic?></div></td>
	<?php
	}
	?>
   </tr>
   ...
   <tr>
	 ...
	 <?php
		foreach ($bulan as $key => $value){
			$totalFCrevclassic = 0;
			foreach ($acclassic as $k => $v){
				$totalFCrevclassic += $FCrev[$v][$value];
			}
	?>
     <td><div align="center" class="style2"><?= $totalFCrevclassic?></div></td>
	<?php
		}
	?>
   </tr>
...
?>
   <tr>
	 ...
	 <?php
	 foreach ($bulan as $key => $value){
		 $totalFCrevclassic = 0;$totaldelayclassic = 0;$totalcancelclassic = 0;$totalDRclassic = 0;
			foreach ($acclassic as $k => $v){
				$totalFCrevclassic += $FCrev[$v][$value];
				$totaldelayclassic += $delay[$v][$value];
				$totalcancelclassic += $cancel[$v][$value];
			}
		$totalDRclassic = round(($totalFCrevclassic - $totaldelayclassic - $totalcancelclassic)/$totalFCrevclassic * 100,2);
	 ?>
     <td><div align="center" class="style2"><?= $totalDRclassic?></div></td>
    <?php
	}
	?>
   </tr>
