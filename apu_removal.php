...
function export_pdf()
  {
        if (document.frm1.periode.value == 0) {
        	   ...
        	   //var str = "excel.php?actype=" + escape(document.frm1.actype.value) + "&weeknum=" + escape(document.frm1.weeknum.value)+ "&year=" + escape(document.frm1.year.value);
                window.open("apurem_pdf_new.php?actype=" + escape(document.frm1.actype.value) + "&periode=" + escape(document.frm1.periode.value) + "&groupby=" + escape(document.frm1.groupby.value) + "&page=" + escape(document.frm1.page.value));
        }
  }
</script>

<p align="center" class="style1 style3">APU REMOVAL</p>
...
$qryFH = "SELECT `tbl_removaldesc`.`Reason`, `tbl_apuremove`.`DateRemove`, `tbl_apuremove`.`Station`, `tbl_apuremove`.`AircraftReg`, `tbl_apuremove`.`SNOff`, `tbl_apuremove`.`TSI`,`tbl_apuremove`.`CSI`, `tbl_apuremove`.`TSN`, `tbl_apuremove`.`CSN`, `tbl_apuremove`.`Remark` FROM `tbl_apuremove` Inner Join `tbl_removaldesc` ON `tbl_apuremove`.`ReasonRemoval` = `tbl_removaldesc`.`ID` WHERE ".$qryline." AND (DATE_FORMAT(`tbl_apuremove`.`DateRemove`,'%Y-%m') BETWEEN '".$lastperiode."' AND '".$periode."') ORDER BY `tbl_apuremove`.`DateRemove` DESC";//die($qryFH);
