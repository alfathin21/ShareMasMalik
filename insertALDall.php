...
//			}
  ...
  for ($i=0;$i<38;$i++)
  {
	...
	//flag05 - next data display Delay
	  ...
	  $ratedel[$bulan[2]][$ATA[$i]] = (isset($FH[$bulan[2]]) && ($FH[$bulan[2]] <> 0)) ? round($ATAdelay[$bulan[2]][$ATA[$i]]/$FC[$bulan[2]]*100,2) : 0;
	  $ratedel[$bulan[1]][$ATA[$i]] = (isset($FH[$bulan[1]]) && ($FH[$bulan[1]] <> 0)) ? round($ATAdelay[$bulan[1]][$ATA[$i]]/$FC[$bulan[1]]*100,2) : 0;
	  $ratedel[$bulan[0]][$ATA[$i]] = (isset($FH[$bulan[0]]) && ($FH[$bulan[0]] <> 0)) ? round($ATAdelay[$bulan[0]][$ATA[$i]]/$FC[$bulan[0]]*100,2) : 0;
	...
	*/
		...
		$qryinsert .= "('$ATAdesc[$i]','ALD','ALD".substr($tahun,2,2).substr($bulan[0],5,2)."','$actype','$ATA[$i]','There ".$tobe." ".$ATAdelay[$bulan[0]][$ATA[$i]]." ".$nominal." during ".bulan($bulan[0])." ".$tahun.". ".$sumdeskripsi."','$sumaction',1,'aryan/','".date('Y-m-d')."','$redalert')";
