...
$panjang = $tinggi*54-$tinggi/2;
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, (array_sum($FHlastyear) <> 0) ? round(($engineremovallastyear["Non Basic UnPlanned"] + $engineremovallastyear["Basic UnPlanned"])/(array_sum($FHlastyear) * $numberofengine)*1000,3) : "",'TBLR',0,'C');
	//die(print_r($FH24));
foreach($bulan as $key => $value)
	{
	...
	$pdf->Cell(60,$tinggi,'      - RATE','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, (array_sum($FHlastyear) <> 0) ? round((($SVlastdata)/(array_sum($FHlastyear)*$numberofengine)*1000),3) : "",'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, (array_sum($FHlastyear) <> 0) ? round((($engineremovallastyear["Basic Planned"] + $engineremovallastyear["Basic UnPlanned"] + $engineremovallastyear["Non Basic Planned"] + $engineremovallastyear["Non Basic UnPlanned"])/(array_sum($FHlastyear)*$numberofengine)*1000),3) : "",'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
