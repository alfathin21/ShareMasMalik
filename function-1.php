...
{
	//$qryFH = "SELECT DATE_FORMAT(tbl_aml.dateAML,'%Y-%m') AS `bulan`, `tbl_aml_detail`.`subjectATA` AS `subjectATA`, Count(`tbl_aml_detail`.`subjectATA`) AS `Jumlah` FROM ((((`tbl_aml` Inner Join `tbl_aml_detail` ON ((`tbl_aml`.`idxAML` = `tbl_aml_detail`.`idxAML`)))))) Inner Join `tbl_masterac` ON `tbl_masterac`.`ACReg` = `tbl_aml`.`acReg` WHERE (DATE_FORMAT(`tbl_aml`.`dateAML`,'%Y-%m') BETWEEN '".$timeawal."' AND '".$timeakhir."') AND `tbl_aml_detail`.`subjectATA` > '20' AND `tbl_masterac`.`ACType` = '".$actype."' AND `tbl_aml`.`PirepMarep` = 'PIREP' group by `tbl_aml_detail`.`subjectATA`, DATE_FORMAT(tbl_aml.dateAML,'%Y-%m') ORDER BY DATE_FORMAT(tbl_aml.dateAML,'%Y-%m') DESC";
...
{
	$alert = "SELECT Count(`tblpirep`.`ATA`) AS `Jumlah`, `tblpirep`.`ATA`  AS `subjectATA`, DATE_FORMAT(`tblpirep`.`DATE`,'%Y-%m') AS `bulan` FROM `tblpirep` WHERE `tblpirep`.`ACTYPE` = '".$actype."' AND `tblpirep`.`DATE` BETWEEN '".$timeawal."' AND '".$timeakhir."' GROUP BY `tblpirep`.`ATA`,  DATE_FORMAT(`tblpirep`.`DATE`,'%Y-%m')";
...
function cumulativeFH12($time,$actype)//query untuk ambil cumulative FH last year
{
...
function cumulativeFC12($time,$actype)//query untuk ambil cumulative FC last year
{
	$qryFH = "SELECT DATE_FORMAT(tbl_monthlyfhfc.MonthEval,'%Y-%m') AS `periode`, `tbl_masterac`.`ACType`, `tbl_masterac`.`ACReg`, `tbl_monthlyfhfc`.`TSN`, `tbl_monthlyfhfc`.`CSN`, `tbl_monthlyfhfc`.`RevFHHours`, `tbl_monthlyfhfc`.`RevFHMin`, `tbl_monthlyfhfc`.`RevFC`, `tbl_monthlyfhfc`.`NoRevFHHours`, `tbl_monthlyfhfc`.`NoRevFHMin`, `tbl_monthlyfhfc`.`NoRevFC` FROM `tbl_masterac` Inner Join `tbl_monthlyfhfc` ON `tbl_masterac`.`IDreg` = `tbl_monthlyfhfc`.`IDReg` WHERE `tbl_masterac`.`ACType` = '".$actype."' AND DATE_FORMAT(tbl_monthlyfhfc.MonthEval,'%Y-%m') <= '".$time."' ORDER BY  DATE_FORMAT(tbl_monthlyfhfc.MonthEval,'%Y-%m') DESC";
	$resultFH = mysql_query($qryFH);
	//die($qryFH);
...
{
	$qryFH = "SELECT `tbl_masterac`.`ACReg`, `tbl_masterac`.`ACType` FROM `tbl_masterac` WHERE `tbl_masterac`.`ACType` = '".$actype."' ORDER BY `tbl_masterac`.`ACReg` ASC";
	$resultFH = mysql_query($qryFH);
	//die($qryFH);
	while ($hasilFH = mysql_fetch_array($resultFH))
	{
...
{
	$qryFH = "SELECT `tbl_removaldesc`.`Reason`, DATE_FORMAT(`tbl_engineremove`.`DateRemove`,'%Y-%m') AS `periode`, Count(`tbl_removaldesc`.`Reason`) AS `jumlah` FROM `tbl_engineremove` Inner Join `tbl_removaldesc` ON `tbl_engineremove`.`ReasonRemoval` = `tbl_removaldesc`.`ID` WHERE `tbl_engineremove`.`AircraftType` = '".$actype."' AND DATE_FORMAT(`tbl_engineremove`.`DateRemove`,'%Y-%m') <= '".$time."' GROUP BY `tbl_removaldesc`.`Reason`,DATE_FORMAT(`tbl_engineremove`.`DateRemove`,'%Y-%m') ORDER BY DATE_FORMAT(`tbl_engineremove`.`DateRemove`,'%Y-%m') DESC";
	$resultFH = mysql_query($qryFH);
	//die($qryFH);
	//while ($hasilFH = mysql_fetch_array($resultFH))
	//{
...
{
	$qryFH = "SELECT `tbl_removaldesc`.`Reason`, DATE_FORMAT(`tbl_engineremove`.`DateRemove`,'%Y-%m') AS `periode`, Count(`tbl_removaldesc`.`Reason`) AS `jumlah` FROM `tbl_engineremove` Inner Join `tbl_removaldesc` ON `tbl_engineremove`.`ReasonRemoval` = `tbl_removaldesc`.`ID` WHERE `tbl_engineremove`.`AircraftType` = '".$actype."' AND (DATE_FORMAT(`tbl_engineremove`.`DateRemove`,'%Y-%m') BETWEEN '".$timeawal."' AND '".$timeakhir."') GROUP BY `tbl_removaldesc`.`Reason`,DATE_FORMAT(`tbl_engineremove`.`DateRemove`,'%Y-%m') ORDER BY DATE_FORMAT(`tbl_engineremove`.`DateRemove`,'%Y-%m') DESC";
	$resultFH = mysql_query($qryFH);
	//die($qryFH);
	//while ($hasilFH = mysql_fetch_array($resultFH))
	//{
...
{
	$qryFH = "SELECT `tbl_ifsd`.`ACType`, DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m') AS `periode`, COUNT(`tbl_ifsd`.`ifsd`) AS `jumlah` FROM `tbl_ifsd` WHERE  `tbl_ifsd`.`ACType` = '".$actype."' AND DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m') <= '".$time."' AND `tbl_ifsd`.`ifsd` = 'IFSD' GROUP BY `tbl_ifsd`.`ACType`,DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m')";
	$resultFH = mysql_query($qryFH);
	//die($qryFH);
	while ($hasilFH = mysql_fetch_array($resultFH))
	{
...
{
	$qryFH = "SELECT `tbl_ifsd`.`ACType`, DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m') AS `periode`, COUNT(`tbl_ifsd`.`ifsd`) AS `jumlah` FROM `tbl_ifsd` WHERE  `tbl_ifsd`.`ACType` = '".$actype."' AND (DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m') BETWEEN '".$timeawal."' AND '".$timeakhir."') AND `tbl_ifsd`.`ifsd` = 'IFSD' GROUP BY `tbl_ifsd`.`ACType`,DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m')";
	$resultFH = mysql_query($qryFH);
	//die($qryFH);
	//while ($hasilFH = mysql_fetch_array($resultFH))
	//{
...
{
	$qryFH = "SELECT `tbl_ifsd`.`ACType`, DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m') AS `periode`, COUNT(`tbl_ifsd`.`ifsd`) AS `jumlah` FROM `tbl_ifsd` WHERE  `tbl_ifsd`.`ACType` = '".$actype."' AND DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m') <= '".$time."' AND `tbl_ifsd`.`rto` = 'RTO' GROUP BY `tbl_ifsd`.`ACType`,DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m')";
	$resultFH = mysql_query($qryFH);
	//die($qryFH);
	while ($hasilFH = mysql_fetch_array($resultFH))
	{
...
{
	$qryFH = "SELECT `tbl_ifsd`.`ACType`, DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m') AS `periode`, COUNT(`tbl_ifsd`.`ifsd`) AS `jumlah` FROM `tbl_ifsd` WHERE  `tbl_ifsd`.`ACType` = '".$actype."' AND (DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m') BETWEEN '".$timeawal."' AND '".$timeakhir."') AND `tbl_ifsd`.`rto` = 'RTO' GROUP BY `tbl_ifsd`.`ACType`,DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m')";
	$resultFH = mysql_query($qryFH);
	//die($qryFH);
	//while ($hasilFH = mysql_fetch_array($resultFH))
	//{
...
function ATApirepperformance($time,$actype)//query untuk  Report
{
	...
	while ($hasilFH = mysql_fetch_array($resultFH))
	{
		$ATA[$hasilFH["subjectATA"]][$hasilFH["acReg"]] = $hasilFH["Jumlah"];
	}
	//$ATA = array_count_values($hour);
	return $ATA;
}
