...
function selectactype() {
	...
	array_push($arractype,"B737-300C","B737-400C","B737-800-E","B737-800-M");
	return $arractype;
}
...
From (yyyy-mm-dd)
  ...
  <input name="startdate" type="text" size="15" maxlength="10" value="<?= $startdate?>"> &nbsp;&nbsp;&nbsp;&nbsp;
To (yyyy-mm-dd)
   
  <input name="enddate" type="text" size="15" maxlength="10" value="<?= $enddate?>"> &nbsp;&nbsp;&nbsp;&nbsp;
...
&nbsp;&nbsp;&nbsp;
  ...
  <?php 
	...
	$arrFH = queryFHdate($startdate,$enddate,$actype);//die(print_r($arrFH));
	$arrFC = queryFCdate($startdate,$enddate,$actype);//die(print_r($arrFC));
	$connectAMSdelay = connectimesysTE();
	$actype = $_REQUEST["actype"];//die($actype);
	$arrdelay = querydelaydate($startdate,$enddate,$actype);//die(print_r($arrFC));
	$arrcancel = querycanceldate($startdate,$enddate,$actype);
	$arrstation = querystationdate($startdate,$enddate,$actype);
 ...
 if ($startdate == "") {
  ...
  <table width="558" border="1">
    ...
    <tr>
	 ...
	 /* for($i=0;$i<12;$i++)
		{*/
			$delay = isset($arrdelay) ? $arrdelay[0] : "0";
			echo "<td align=\"center\" width=\"20\">".$delay."</td>";
		//}
	  ?>
    </tr>
    <tr>
	  ...
	  /*for($i=0;$i<12;$i++)
		{*/
			$cancel = isset($arrcancel) ? $arrcancel[0] : "0";
    ...
    <tr>
	  ...
	  /*for($i=0;$i<12;$i++)
		{*/
			$station = isset($arrstation) ? $arrstation[0] : "0";
    ...
    <tr>
	  ...
	  //for($i=0;$i<11;$i++)
		//{
			$TORev = isset($arrFC) ? $arrFC[0] : 0;
			echo "<td align=\"center\">".$TORev."</td>";
		//}
    ...
    <tr>
	  ...
	  //for($i=0;$i<11;$i++)
	//	{
			$FhRev = isset($arrFH) ? $arrFH[0] : 0;
			echo "<td align=\"center\">".$FhRev."</td>";
	//	}
	  ?>
	...
	//	{
			...
			$DR = ($TORev <> 0) ? round(($TORev - $dc)/$TORev * 100,2) : 0;
			echo "<td align=\"center\">".$DR."</td>";
	//	}
	  ?>
