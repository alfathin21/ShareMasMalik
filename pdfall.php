...
function BasicTable($header,$actype,$periode)
{
    ...
    //    $this->Cell(40,17,$col,10);
		...
		$this->Ln();
		//editing untuk B737-800-F/M/N
		if($actype == "B737-800-M") {
			$judulAC = 'B737-800-F/M/N';
		} else {
			$judulAC = $actype;
		}
		$this->Cell(275,7,(($actype == "ATR72-600") || ($actype == "ATR72-600-GA") || ($actype == "ATR72-600-QG")? "": (($actype == "A330-300") || ($actype == "A330-200") || ($actype == "A320-200") || ($actype == "A330") ? "AIRBUS" : (($actype == "CRJ-1000") ? "BOMBARDIER" :  "BOEING")))." ".$judulAC,'LRB',0,'C');
		//$this->Cell(275,7,(($actype == "ATR72-600") ? "" :(($actype == "A330-300") || ($actype == "A330-200") || ($actype == "A320-200") || ($actype == "A330") ? "AIRBUS" : (($actype == "CRJ-1000") ? "BOMBARDIER" :  "BOEING")))." ",(($actype == "B737-800-M") ? "B737-800-F/M/N" :  .$actype),'LRB',0,'C');		
		$this->Ln();
...
function delayrate($bulan,$delayratelastyear,$delayrate,$techdelay,$FCrev){
	$this->Cell(53,6,'    - Rate / 100 Take Off','LBRT',0,'L',0);
	$this->Cell(15,6,round($delayratelastyear,2),'LBR',0,'C',0);
	foreach ($bulan as $k => $value){
		$this->Cell(15,6,($FCrev[$value] != 0) ? round($delayrate[$value],2) : "",'TLRB',0,'C',($FCrev[$value] != 0) ? 0 : 1);
	}
	$this->Cell(27,6,round(array_sum($techdelay)/array_sum($FCrev)*100,2),'LBR',0,'C');
	$this->Ln();
}
...
function dispatch($bulan,$FCrevlastyear,$techdelaylastyear,$cancellastyear,$FCrev,$techdelay,$cancel,$FH){
    ...
    //    $this->Cell(40,17,$col,10);
		$this->SetY(30);
/* 		if ($actype = 'A320-200') {
			$this->Cell(20,5,'QG','LBT',0,'C');
		}
		else if ($actype = 'B737-CL') {
			$this->Cell(20,5,'QG','LBT',0,'C');
		}
		else {
			$this->Cell(20,5,'GA','LBT',0,'C');
		} */
		if ($actype == 'A320-200') {
		$this->Cell(45,5,'QG '.$actype,'LBT',0,'C');
		} else if ($actype == 'B737-CL') {
		$this->Cell(45,5,'QG '.$actype,'LBT',0,'C');
		} else if ($actype == 'B737-800-M') {
		$this->Cell(45,5,'GA B737-800-F/M/N','LBT',0,'C');
		}  else {
		$this->Cell(45,5,'GA '.$actype,'LBT',0,'C');
		}
		//$this->Cell(45,5,$actype,'LBT',0,'C');
	...
	$this->Ln();
    $this->Cell(185,4,'The Alert Level (AL) is based on monthly Technical Pilot Report / Maintenance Finding Report / Delay Rate of last Four Quarters (Average + 2 *STD)',0,0,'');
	$this->Ln();
    //$this->SetY(-35);
    ...
    //$this->SetY(-20);
	$this->Ln();
	//$this->Cell(170,10,'issued by JKTMQGA dan Compiled by GMF Reliability Engineering & Services',0,0,'R');	
	}
...
function data($bulan,$ATApirep,$ATA12bulan,$alertlevel,$ATA,$ATAdesc,$FH,$FH12,$pembagi,$actype,$preselectpirep){
	...
	for ($i=0;$i<38;$i++) {
		...
		if($actype == 'B737-CL')
		{
			$this->Cell(11,4.5,"",'TBLR',0,'C',1);
			$this->Cell(12,4.5,"",'TBLR',0,'C',1);
...
function JudulCumulative($header,$actype,$periode,$tinggi,$reghal)
{
    ...
    //    $this->Cell(40,17,$col,10);
		...
		$this->Cell(272,7,(($actype == "A330-300") || ($actype == "A330-200") || ($actype == "A320-200") || ($actype == "A330")? "AIRBUS" : (($actype == "CRJ-1000") ? "BOMBARDIER" : (($actype == "ATR72-600") || ($actype == "ATR72-600-GA") || ($actype == "ATR72-600-QG") ? "" : "BOEING")))." ".$judulAC,'LRB',0,'C');
		$this->Ln();
...
$L = 14;
	...
	$this->Cell(60,4,(($actype == "A330-300") || ($actype == "A330-200") || ($actype == "A320-200") || ($actype == "A330")? "AIRBUS" : (($actype == "CRJ-1000") ? "BOMBARDIER" : (($actype == "ATR72-600") ? "" : "BOEING"))).' '.$judulAC,1,0,'C');
	$this->Cell($L,4,'',1,0,'C');
...
//fungsi header engine removal detail
function HeaderEngRem()
{
	$i1 = 14;
    //Logo
	/*$this->SetLeftMargin(25);
...
//fungsi table buat engine removal detail
function BasicTableEngRem($header,$actype,$periode)
{
		$this->SetY(20);
		$this->SetX(10);
	    $this->Cell(275,7,$header,'LRT',0,'C');
...
$this->_out($s);
  ...
  $bulanalert = substr($periode,-2,2);
	...
	if ($actype == "B737-Classic") {
		$qrylineincident = "`tbl_sdr`.`ACTYPE` IN ('B737-300','B737-400','B737-500','B737-CL')";
	} else if ($actype == "B737-Classic GA") {
		$qrylineincident = "`tbl_sdr`.`ACTYPE` IN ('B737-300','B737-400','B737-500')";
	} else if ($actype == "ATR72-600-GA") {
		$qrylineincident = "`tbl_sdr`.`Reg` Like 'PK-GA%'";
	} else if ($actype == "ATR72-600-QG") {
		$qrylineincident = "`tbl_sdr`.`Reg` Like 'PK-GJ%'";			
	} else if ($actype == "B737-800-E") {
		$qrylineincident = "`tbl_sdr`.`Reg` Like 'PK-GE%'";
	} else if ($actype == "B737-800-M") {
		$qrylineincident = "(`tbl_sdr`.`Reg` Like 'PK-GM%' or `tbl_sdr`.`Reg` Like 'PK-GF%')";
	} else if ($actype == "A330") {
		$qrylineincident = "`tbl_sdr`.`ACTYPE` IN ('A330-200','A330-300')";
	} else {
		$qrylineincident = "`tbl_sdr`.`ACTYPE` IN ('$actype')";
	}	
	
	$qryincident = "SELECT Count(`tbl_sdr`.`ACTYPE`) AS `jumlah`, DATE_FORMAT(`tbl_sdr`.`DateOccur`,'%Y-%m') AS `bulan` FROM `tbl_sdr` WHERE ".$qrylineincident." AND (DATE_FORMAT(`tbl_sdr`.`DateOccur`,'%Y-%m') BETWEEN '".$bulan[0]."' AND '".$bulan[11]."') GROUP BY DATE_FORMAT(`tbl_sdr`.`DateOccur`,'%Y-%m')";
	...
	$qryincidentlastyear = "SELECT Count(`tbl_sdr`.`ACTYPE`) AS `jumlah`, DATE_FORMAT(`tbl_sdr`.`DateOccur`,'%Y-%m') AS `bulan` FROM `tbl_sdr` WHERE ".$qrylineincident." AND (DATE_FORMAT(`tbl_sdr`.`DateOccur`,'%Y-%m') BETWEEN '".(substr($periode,0,4) - 1)."-01' AND '".(substr($periode,0,4) - 1)."-12') GROUP BY DATE_FORMAT(`tbl_sdr`.`DateOccur`,'%Y-%m')";
	...
	if (is_array ($ATApirepgraph) || is_object ($ATApirepgraph)){
	...
	//marep graph start here (?)
		$ATAmareplastyear = ATAmar($awallastyear,$akhirlastyear,$actype);//print_r($ATAmareplastyear);
	if (is_array($ATAmareplastyear)){
	foreach ($ATAmareplastyear as $k => $v){
		$maintreportratelast[substr($k,-2,2)-1] = round(array_sum($v)/$FH24[$k] * 1000,2);
		$jumlahmarep[$k] = array_sum($v);
		//$kuncibaru[substr($k,-2,2)] = 
	}
	$maintreportratelast = array_reverse($maintreportratelast,true);
	} else {
		$maintreportratelast[0] = 0;
	}
	for($i=0;$i<12;$i++){
		$maintreportratelast[$i] = isset($maintreportratelast[$i]) ? $maintreportratelast[$i] : 0;
	}
	ksort($maintreportratelast);
	
	//die(print_r($maintreportratelast));
	$ATAmarep = ATAmar($bulan[0],$bulan[11],$actype);//die(print_r($ATAmarep));
	$ATAmarepgraph = ATAmar(substr($periode,0,4),$periode,$actype);//die(print_r($ATAmarepgraph));
	if (is_array ($ATAmarepgraph) || is_object($ATAmarepgraph)){
	foreach ($ATAmarepgraph as $k => $v){
		$marepcurrentrate[substr($k,-2,2)-1] = round(array_sum($v)/$FH24[$k] * 1000,2);
		$axismarep[substr($k,-2,2)-1] = round(array_sum($v),0);
	}//print_r($axismarep);
	}
	for($i=0;$i<12;$i++){
		$marepcurrentrate[$i] = isset($marepcurrentrate[$i]) ? $marepcurrentrate[$i] : 0;
		$axismarep[$i] = isset($axismarep[$i]) ? $axismarep[$i] : "";
	}
	ksort($marepcurrentrate);ksort($axismarep);
	
	//marep graph end here (?)
	...
	if ($actype == "B737-Classic") {
		...
		$qryline = "`tbl_sdr`.`Reg` Like 'PK-GA%'";
	} else if ($actype == "ATR72-600-QG") {
		$qryline = "`tbl_sdr`.`Reg` Like 'PK-GJ%'";		
	} else if ($actype == "B737-800-E") {
		$qryline = "`tbl_sdr`.`Reg` Like 'PK-GE%'";
	} else if ($actype == "B737-800-M") {
		$qryline = "(`tbl_sdr`.`Reg` Like 'PK-GM%' or `tbl_sdr`.`Reg` Like 'PK-GF%' or `tbl_sdr`.`Reg` Like 'PK-GN%')";
	} else if ($actype == "B737-800-M") {
		$qryline = "(`tbl_sdr`.`Reg` Like 'PK-GM%' or `tbl_sdr`.`Reg` Like 'PK-GF%' or `tbl_sdr`.`Reg` Like 'PK-GN%' or `tbl_sdr`.`Reg` Like 'PK-GD%')";		
	} else {
	...
	//	$avgduration[$v] = isset($delaydurationmenit[$v]) ? round($delaydurationmenit[$v] / $techdelay[$v],0) : 0;
		...
		$cancel[$v] = isset($cancel[$v]) ? $cancel[$v] : 0;
		if(($actype == "B737-CL") or ($actype == "B747-400")) {
			if($FCrev[$v] == 0)
			{$dispathcrel[] = 0;
			$dispathcrel2[] = targetdispatch($actype);
			$xaxis[] = bulan($v);		
			} else {
			$dispathcrel[] = (isset($FCrev[$v])) ? round(($FCrev[$v] - $techdelay[$v] - $cancel[$v])/$FCrev[$v] * 100,2) : 0;
			$dispathcrel2[] = targetdispatch($actype);
			$xaxis[] = bulan($v);
			}
		} else {
	...
	$qryifsd = "SELECT `tbl_ifsd`.`ACType`, DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m') AS `periode`, COUNT(`tbl_ifsd`.`ifsd`) AS `jumlah` FROM `tbl_ifsd` WHERE  ".$qryline." AND DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m') <= '".$periode."' AND `tbl_ifsd`.`ifsd` = 'IFSD' GROUP BY `tbl_ifsd`.`ACType`,DATE_FORMAT(`tbl_ifsd`.`DateOccur`,'%Y-%m')";
...
// penambahan fungsi Report Untuk Sriwijaya
    else if ($actype == "737-500-IN"){
        $enginetype = "CFM56-3";            
    }else if ($actype == "ATR72600-IN"){
        $enginetype = "PW127M";         
    } else if ($actype == "737-800-SJ"){
        $enginetype = "CFM56-7";            
    }  else if ($actype == "737-900-SJ"){
        $enginetype = "CFM56-7";            
    }  
    else {
        $enginetype = "CFM56-3";
...
$pdf->delayrate($bulan,$delayratelastyear,$delayrate,$techdelay,$FCrev);
...
$pdf->note();
if ($actype == "A320-200") {
	$pdf->Cell(170,10,'Issued by Citilink Engineering & Maintenance and Compiled by GMF Reliability Engineering & Services',0,0,'R');	
}
else if ($actype == "B737-CL") {
	$pdf->Cell(170,10,'Issued by Citilink Engineering & Maintenance and Compiled by GMF Reliability Engineering & Services',0,0,'R');	
}
else {
	$pdf->Cell(170,10,'Issued by JKTMQGA and Compiled by GMF Reliability Engineering & Services',0,0,'R');	
}
$pdf->SetY(265);
$pdf->Cell(187,6,"Page : ".$page."3-1",0,0,'R');
...
$pdf->Line(10,225,192,225);//garis mendatar no 6

$pdf->Line(10,30,10,225);//garis vertikal paling kiri (no 1)
$pdf->Line(50,42,50,225);//garis vertikal no 2
$pdf->Line(102,42,102,225);//garis vertikal no 3
$pdf->Line(157,46,157,225);//garis vertikal no 4
$pdf->Line(192,30,192,225);//garis vertikal no 5

//lembar marep dimulai dimari
$pdf->SetLineWidth(0);
$pdf->AddPage('P');
$pdf->head($actype,$FH,$FH12,$bulan,'TOTAL FLIGHT HOURS','MAINTENANCE FINDING REPORT',$tahun);
$pdf->data($bulan,$ATAmarep,$ATAmarep12bulan,$alertlevelmarep,$ATA,$ATAdesc,$FH,$FH12,1000,$actype,$preselectmarep);
$pdf->note();
if ($actype == "A320-200") {
	$pdf->Cell(170,10,'Issued by Citilink Engineering & Maintenance and Compiled by GMF Reliability Engineering & Services',0,0,'R');	
}
...
$pdf->SetY(265);
$pdf->Cell(187,6,"Page : ".$page."3-2",0,0,'R');
$pdf->SetFont('Arial','B',7);
...
$pdf->Line(10,46,192,46);//garis mendatar no 4
$pdf->Line(50,54,192,54);//garis mendatar no 5
$pdf->Line(10,229.5,192,229.5);

$pdf->Line(10,30,10,229.5);
$pdf->Line(50,42,50,229.5);
$pdf->Line(102,42,102,229.5);
$pdf->Line(157,46,157,229.5);
$pdf->Line(192,30,192,229.5);
//$pdf->AddPage();
...
$header=$judulAC.' Reliability Graph';
//Data loading
...
//BUAT MILIH FILE GRAFIS APA ; 747 atau PESAWAT LAIN
if(($actype == "B747-400")) {
	include('DRgrafik747.php');
} else if(($actype == "B737-CL")) {
	include('DRgrafikCL.php');
} else {
...
$pdf->Text(180,280,'Page : '.$page.'5 - 2');
...
$panjang = $tinggi*54-$tinggi/2;
	...
	foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi, (!empty($FHeng12) ? round((($ifsd[$bulan12[$key+1]] + $ifsd[$bulan12[$key+2]] + $ifsd[$bulan12[$key+3]] + $ifsd[$bulan12[$key+4]] + $ifsd[$bulan12[$key+5]] + $ifsd[$bulan12[$key+6]] + $ifsd[$bulan12[$key+7]] + $ifsd[$bulan12[$key+8]] + $ifsd[$bulan12[$key+9]] + $ifsd[$bulan12[$key+10]] + $ifsd[$bulan12[$key+11]] + $ifsd[$bulan12[$key+12]])/($FH24[$bulan12[$key+1]] + $FH24[$bulan12[$key+2]] + $FH24[$bulan12[$key+3]] + $FH24[$bulan12[$key+4]] + $FH24[$bulan12[$key+5]] + $FH24[$bulan12[$key+6]] + $FH24[$bulan12[$key+7]] + $FH24[$bulan12[$key+8]] + $FH24[$bulan12[$key+9]] + $FH24[$bulan12[$key+10]] + $FH24[$bulan12[$key+11]] + $FH24[$bulan12[$key+12]]) * 1000),3) : 0),'TBLR',0,'C');
	}
	*/
	$pdf->SetFont('Arial','B',7);
	$pdf->Ln();
	...
	$pdf->Cell(60,$tinggi,'BASIC - PLANNED ENGINE REMOVALS','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	...
	$pdf->Ln();
	//$pdf->Cell(10,$tinggi,'','LR',0,'L');
	$pdf->Cell(60,$tinggi,'            - UNPLANNED ENGINE REMOVALS','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	...
	$pdf->Ln();
	//$pdf->Cell(10,$tinggi,'','LR',0,'L');
	$pdf->Cell(60,$tinggi,'NON BASIC - PLANNED ENGINE REMOVALS','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
...
//COBA TAMBAH ENGREM DETAIL DIBAWAH INI  YA


/* $pdf->AddPage();
include ('engrem_pdf_new_all.php');
$pdf->Image($fileName2,30,40);
	$pdf->Ln();
$pdf->Text(260,190,'Page : '.$page.'9'); */
