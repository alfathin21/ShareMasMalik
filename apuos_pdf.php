...
require('fpdf/fpdf.php');
	...
	$manufacture = (($judulAC == "A330-200") || ($judulAC == "A330-300") || ($judulAC == "A320-200") || ($judulAC == "A330")? "AIRBUS" : (($judulAC == "CRJ-1000") ? "BOMBARDIER" :(($judulAC == "ATR72-600") ? "" : "BOEING")))." ".((strpos($judulAC,",")) ? "B737-Classic" : $judulAC);
...
$panjang = $tinggi*54-$tinggi/2;
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi,(isset($FHapulastyear)) ? (round(array_sum($FHapulastyear) * 1,0)) : 0,'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi,($FHapu[$value]) ? (round($FHapu[$value] * 1,0)) : 0,'TBLR',0,'C');
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi,(isset($FCapulastyear)) ? (array_sum($FCapulastyear)) : 0,'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi,($FCapu[$value] <> 0) ? (($FCapu[$value])) : 0,'TBLR',0,'C');
...
$getRY = $pdf->GetY();
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, (isset($FHapulastyear)) ? round(($engineremovallastyear["Non Basic UnPlanned"] + $engineremovallastyear["Basic UnPlanned"])/(array_sum($FHapulastyear) * 1)*1000,3) : "",'TBLR',0,'C');
	//die(print_r($FH24));
foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi, (!empty($FHapu[$value]) ? round((($engineremoval["Basic UnPlanned"][$value] + $engineremoval["NonBasic UnPlanned"][$value])/($FHapu[$value] * 1)*1000),3) : 0),'TBLR',0,'C');

		$uer[] =  !empty($FHapu[$value]) ? round((($engineremoval["Basic UnPlanned"][$value] + $engineremoval["NonBasic UnPlanned"][$value])/($FHapu[$value] * 1)*1000),3) : 0;
	}
	...
	foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi, (!empty($FHapu24[$bulan12[$key+10]]) ? round((($engineremoval["Basic UnPlanned"][$bulan12[$key+10]] + $engineremoval["Non Basic UnPlanned"][$bulan12[$key+10]] + $engineremoval["Basic UnPlanned"][$bulan12[$key+11]] + $engineremoval["Non Basic UnPlanned"][$bulan12[$key+11]] + $engineremoval["Basic UnPlanned"][$bulan12[$key+12]] + $engineremoval["Non Basic UnPlanned"][$bulan12[$key+12]])/(($FH24[$bulan12[$key+10]] + $FHapu24[$bulan12[$key+11]] + $FHapu24[$bulan12[$key+12]]) * 1)*1000),3) : 0),'TBLR',0,'C');
	}
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, (isset($FHapulastyear)) ? round((($SVlastdata)/(array_sum($FHapulastyear)*1)*1000),3) : "",'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		$shopvisitrate[$value] = !empty($FHapu[$value]) ? round((($SV[$value])/($FHapu[$value] * 1)*1000),3) : 0;
	...
	foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi, (!empty($FHapu24[$bulan12[$key+10]]) ? round((($SV[$bulan12[$key+10]] + $SV[$bulan12[$key+11]] +  $SV[$bulan12[$key+12]])/(($FHapu24[$bulan12[$key+10]] + $FHapu24[$bulan12[$key+11]] + $FHapu24[$bulan12[$key+12]]) * 1)*1000),3) : 0),'TBLR',0,'C');
	}
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, (isset($FHapulastyear)) ? round((($engineremovallastyear["Basic Planned"] + $engineremovallastyear["Basic UnPlanned"] + $engineremovallastyear["Non Basic Planned"] + $engineremovallastyear["Non Basic UnPlanned"])/(array_sum($FHapulastyear)* 1)*1000),3) : "",'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		$engremovalrate[$value] = !empty($FHapu[$value]) ? round((($engineremoval["Basic Planned"][$value] + $engineremoval["Basic UnPlanned"][$value] + $engineremoval["Non Basic Planned"][$value] + $engineremoval["Non Basic UnPlanned"][$value])/($FHapu[$value] * 1)*1000),3) : 0;
	...
	for($i=1;$i<13;$i++)
	{
		...
		$FHengremove += $FHapu24[$bulan12[$key+$i]];
	}
	$rate = !empty($FHengremove) ? round(($predbasplan + $predbasunplan + $prednonbasplan + $prednonbaseunplan)/($FHengremove * 1)*1000,3) : 0;
