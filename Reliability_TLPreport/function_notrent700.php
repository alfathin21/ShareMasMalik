...
function ATA($timeawal,$timeakhir,$actype)//query untuk  Report
{
	if ($actype == "B737-Classic") {
		...
		$qryline = "(`tblpirep`.`REG` Like 'GM%' or `tblpirep`.`REG` Like 'GF%' or `tblpirep`.`REG` Like 'GN%')";
	} else {
		$qryline = "`tblpirep`.`ACTYPE` IN ('$actype')";
	}
	
	$qryFH = "SELECT Count(`tblpirep`.`ATA`) AS `Jumlah`, `tblpirep`.`ATA`  AS `subjectATA`, DATE_FORMAT(`tblpirep`.`DATE`,'%Y-%m') AS `bulan` FROM `tblpirep` WHERE ".$qryline." AND (DATE_FORMAT(`tblpirep`.`DATE`,'%Y-%m') BETWEEN '".$timeawal."' AND '".$timeakhir."') GROUP BY `tblpirep`.`ATA`,  DATE_FORMAT(`tblpirep`.`DATE`,'%Y-%m') ORDER BY DATE_FORMAT(`tblpirep`.`DATE`,'%Y-%m') DESC";
	$resultFH = mysql_query($qryFH);
	//die($qryFH);
	while ($hasilFH = mysql_fetch_array($resultFH))
	{
...
function ATA12($timeawal,$timeakhir,$actype)////query untuk ATA 12 Month Pilot Report 
{
	...
	$qryFH = "SELECT Count(`tblpirep`.`ATA`) AS `Jumlah`, `tblpirep`.`ATA` AS `subjectATA` FROM `tblpirep` WHERE ".$qryline." AND DATE_FORMAT(`tblpirep`.`DATE`,'%Y-%m') BETWEEN '".$timeawal."' AND '".$timeakhir."' GROUP BY `tblpirep`.`ATA`";
	$resultFH = mysql_query($qryFH);
	//die($qryFH);
	while ($hasilFH = mysql_fetch_array($resultFH))
	{
...
function alert($timeawal,$timeakhir,$actype)//query untuk Alert Level Pilot Report
{
	...
	$alert = "SELECT Count(`tblpirep`.`ATA`) AS `Jumlah`, `tblpirep`.`ATA`  AS `subjectATA`, DATE_FORMAT(`tblpirep`.`DATE`,'%Y-%m') AS `bulan` FROM `tblpirep` WHERE ".$qryline." AND (DATE_FORMAT(`tblpirep`.`DATE`,'%Y-%m') BETWEEN '".$timeawal."' AND '".$timeakhir."') GROUP BY `tblpirep`.`ATA`,  DATE_FORMAT(`tblpirep`.`DATE`,'%Y-%m')";
