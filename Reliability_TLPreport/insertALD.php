<?php
  ...
  for ($i=0;$i<38;$i++)
  {
	...
	//die($sumaction);
		...
		$qryinsert .= "('$ATAdesc[$i]','ALD','ALD".substr($tahun,2,2).substr($bulan[0],5,2)."',if('$actype'='B737-Classic GA','B737-Classic','$actype'),'$ATA[$i]','There ".$tobe." ".$newATAdelay[$bulan[0]][$ATA[$i]]." ".$nominal." during ".bulan($bulan[0])." ".$tahun.". ".$sumdeskripsi."','$sumaction',1,'aryan/','".date('Y-m-d')."','$redalert')";
