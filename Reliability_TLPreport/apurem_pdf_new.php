...
function Header()
{
	...
	//$this->SetX(50);
	$this->Cell(264,5,'APU REMOVAL','',1,'C');
	...
	$this->Cell(80,10,'APU SERIAL NUMBER','LRTB',2,'C');
	...
	$this->Line(35,23,250,23);
	$this->Line(35,49,250,49);
	
	$this->Line(35,23,35,49);
	$this->Line(250,23,250,49);
...
if ($actype == "B737-Classic") {
		...
		$qryline = "(`tbl_apuremove`.`AircraftReg` Like 'PK-GM%' or `tbl_apuremove`.`AircraftReg` Like 'PK-GF%')";
	} else if ($actype == "A330") {
		$qryline = "`tbl_apuremove`.`AircraftType` IN ('A330-200','A330-300')";			
	} else {
		$qryline = $qryline = "`tbl_apuremove`.`AircraftType` IN ('$actype')";
	}
$qryFH = "SELECT `tbl_removaldesc`.`Reason`, `tbl_apuremove`.`DateRemove`, `tbl_apuremove`.`Station`, `tbl_apuremove`.`AircraftReg`, `tbl_apuremove`.`SNOff`, `tbl_apuremove`.`TSI`,`tbl_apuremove`.`CSI`, `tbl_apuremove`.`TSN`, `tbl_apuremove`.`CSN`, `tbl_apuremove`.`Remark` FROM `tbl_apuremove` Inner Join `tbl_removaldesc` ON `tbl_apuremove`.`ReasonRemoval` = `tbl_removaldesc`.`ID` WHERE ".$qryline." AND (DATE_FORMAT(`tbl_apuremove`.`DateRemove`,'%Y-%m') BETWEEN '$lastperiode' AND '".$_REQUEST["periode"]."') ORDER BY `tbl_apuremove`.`DateRemove` DESC";//die($qryFH);
