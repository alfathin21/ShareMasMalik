...
foreach ($get as $a => $b){
	...
	if (($newget[substr($k,-8)][0] <> "") && ($newget[substr($k,-8)][1] <> "") && ($newget[substr($k,-8)][2] <> ""))
	{	
		mysqli_query($coninsert,"INSERT INTO tblflweng (Eventid, nopeg, Status, unit) VALUES ('".substr($k,-8)."', '".$newget[substr($k,-8)][2]."', 'Open', '".$newget[substr($k,-8)][0]."')");
		
		$resultinsert = mysqli_query($coninsert,"SELECT TDAMNo FROM tdamx WHERE ID = '".substr($k,-8)."'");
		while ($hsl = mysqli_fetch_array($resultinsert)){
			$tdamnno = $hsl["TDAMNo"];
		}
		mysqli_query($coninsert,"INSERT INTO tbltdameng (Eventid, tdamno, Nopeg, unit, Date) VALUES ('".substr($k,-8)."', '".$tdamnno."', '".$newget[substr($k,-8)][1]."', '".$newget[substr($k,-8)][0]."', '".date('Y-m-d')."')");
	}
}
		...
		<?php
			...
			while ($row = mysqli_fetch_array($result))
			{
				...
				//{
					...
					unset($prob);
					$resultprob = mysqli_query($connect,$queryprob) or die("data Problem gagal");
					...
					unset($rect);
					$resultrec = mysqli_query($connect,$queryrec);
					...
					$resulttdamx = mysqli_query($connect,$qrytdamx);
