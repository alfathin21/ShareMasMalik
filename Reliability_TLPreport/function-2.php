...
function alert($timeawal,$timeakhir,$actype)//query untuk Alert Level Pilot Report
{
	...
	while ($hasilalert = mysql_fetch_array($resultalert))
	{
		...
		$bulanan[] = $hasilalert["bulan"];
	}
	mysql_free_result($resultalert);
	$bulanan = array_keys(array_count_values($bulanan));
	sort($bulanan);//die(print_r($bulanan));
	...
	foreach ($ATAalert as $key => $value){
		...
		//die(print_r($rate));
		$mean = array_sum($rate)/12;//die($mean);
		$sigma_temp = 0;
		...
		for ($i=0;$i<12;$i++)
		{
			$rate[$bulanan[$i]] = isset($rate[$bulanan[$i]]) ? $rate[$bulanan[$i]] : 0;
			$sigma_temp += ($rate[$bulanan[$i]] - $mean) * ($rate[$bulanan[$i]] - $mean);
			//$kurang[$key][$value[$i]] = ($value[$i] - $mean) * ($value[$i] - $mean);
		}
		$standard_deviation = sqrt($sigma_temp / 12);
		$alertlevel[$key] = $mean + (2 * $standard_deviation);//die(print_r($alertlevel));
	}
...
function alertdelay($timeawal,$timeakhir,$actype)//query untuk Alert Level Delay Report
{
	...
	foreach ($ATAalert as $key => $value){
		...
		$standard_deviation = sqrt($sigma_temp / 12);
		$alertlevel[$key] = $mean + (2 * $standard_deviation);
	}
