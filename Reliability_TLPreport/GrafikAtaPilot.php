...
<?php
	...
	$FH = queryFH12($periode,$actype,24);
	...
	foreach ($bulan as $key => $value){
		...
		$alertlevel[$value] = alert($timeatas[$value],$timebawah[$value],$actype);
	}
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1819 class=xl3013144
 ...
 <tr height=18 style='height:13.5pt'>
  ...
  height:13.5pt'>TOTAL<span style='mso-spacerun:yes'>  </span>FLIGHT HOURS</td>
<?php
foreach ($bulan as $key => $value){
?>
  <td class=xl3413144 style='border-left:none' x:num><?= $FH[$value]?></td>
<?php
}
?>
...
?>
 ...
 <tr height=17 style='height:12.75pt'>
  ...
  <?php
	  foreach ($bulan as $key => $value){
  ?>
  <td class=xl3313144 align=right x:num><?= round(($ATAdata[$value][$v]/$FH[$value] * 1000),2)?></td>
  <?php
	}
  ?>
  <td class=xl3013144></td>
