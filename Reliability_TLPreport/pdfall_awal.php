...
function BasicTable($header,$actype,$periode)
{
    ...
    //    $this->Cell(40,17,$col,10);
		...
		$this->Ln();
		$this->Cell(275,7,(($actype == "A330-300") || ($actype == "A330-200") ? "AIRBUS" : "BOEING")." ".$actype,'LRB',0,'C');
		$this->Ln();
...
function fleet($bulan,$fleet,$fleetlastyear,$FH,$availablelastyear){
	...
	$this->Cell(15,6,(array_sum($availablelastyear) <> 0) ? round(array_sum($fleetlastyear)/12,0) : 0,'LBR',0,'C',0);
	foreach ($bulan as $k => $v){
...
function fhperfc($bulan,$FH,$FHlastyear,$FC,$FClastyear,$text){
	...
	$fhperfclastyear = (array_sum($FClastyear) <> 0) ? ((array_sum($FHlastyear) - (array_sum($FHlastyear) % array_sum($FClastyear)))/array_sum($FClastyear))." : ".substr("0".(round(((array_sum($FHlastyear) % array_sum($FClastyear))/array_sum($FClastyear)*60),0)),-2,2) : 0;
...
function dailyfh($bulan,$FHlastyear,$availablelastyear,$FH,$available,$text){
	$this->Cell(53,6,$text,'LBRT',0,'L',0);
	$dailyfhlastyear = (array_sum($availablelastyear) <> 0) ? ((array_sum($FHlastyear)-(array_sum($FHlastyear)%array_sum($availablelastyear)))/array_sum($availablelastyear))." : ".substr("0".round(((array_sum($FHlastyear)%array_sum($availablelastyear))/array_sum($availablelastyear)*60),0),-2,2) : 0;
...
function dailyfc($bulan,$FClastyear,$availablelastyear,$FC,$available,$text){
	...
	$dailyfclastyear = (array_sum($availablelastyear) <> 0) ? round(array_sum($FClastyear)/array_sum($availablelastyear),2) : 0;
...
function incidentrate($bulan,$jumlahincidentlastyear,$incident,$FHlastyear,$FH){
	...
	$this->Cell(15,6,(array_sum($FHlastyear) <> 0) ? round(($jumlahincidentlastyear)/array_sum($FHlastyear)*1000,2) : 0,'LBR',0,'C',0);
	foreach ($bulan as $k => $value){
...
function dispatch($bulan,$FCrevlastyear,$techdelaylastyear,$cancellastyear,$FCrev,$techdelay,$cancel,$FH){
	...
	$this->Cell(15,6,(array_sum($FCrevlastyear) <> 0) ? round((array_sum($FCrevlastyear) - array_sum($techdelaylastyear) - $cancellastyear)/array_sum($FCrevlastyear) * 100,2) : "100",'LBR',0,'C',0);
	foreach ($bulan as $k => $value){
...
function data($bulan,$ATApirep,$ATA12bulan,$alertlevel,$ATA,$ATAdesc,$FH,$FH12,$pembagi,$actype,$preselectpirep){
	...
	for ($i=0;$i<35;$i++) {
		...
		$this->Cell(11,4.5,round($alertlevel[$ATA[$i]],2),'TBLR',0,'C',0);
		$this->Cell(12,4.5,(($FH[$bulan[11]] <> 0) && ($FH[$bulan[10]] <> 0) && ($FH[$bulan[9]] <> 0)) ? alertstatus(round($ATApirep[$bulan[9]][$ATA[$i]]/$FH[$bulan[9]]*$pembagi,2),round($ATApirep[$bulan[10]][$ATA[$i]]/$FH[$bulan[10]]*$pembagi,2),round($ATApirep[$bulan[11]][$ATA[$i]]/$FH[$bulan[11]]*$pembagi,2),$alertlevel[$ATA[$i]]) : "",'TBLR',0,'C',0);
...
//	$this->Cell(14,4.5,round(($ATApirep[$bulan[9]]['OT'] + $ATApirep[$bulan[10]]['OT'] + $ATApirep[$bulan[11]]['OT'])/($FH[$bulan[9]] + $FH[$bulan[10]] + $FH[$bulan[11]])*$pembagi,2),'TBLR',0,'C');
	...
	//Alert Level
	$this->Cell(11,4.5,round($alertlevel['OT'],2),'TBLR',0,'C',0);
	//STATUS
	$this->Cell(12,4.5,(($FH[$bulan[9]] != 0) && ($FH[$bulan[10]] != 0) && ($FH[$bulan[11]])) ? alertstatus(round($ATApirep[$bulan[9]]['OT']/$FH[$bulan[9]]*$pembagi,2),round($ATApirep[$bulan[10]]['OT']/$FH[$bulan[10]]*$pembagi,2),round($ATApirep[$bulan[11]]['OT']/$FH[$bulan[11]]*$pembagi,2),$alertlevel['OT']) : "",'TBLR',0,'C',0);
	//TREND
...
function JudulCumulative($header,$actype,$periode,$tinggi,$reghal)
{
    ...
    //    $this->Cell(40,17,$col,10);
		...
		$this->Ln();
		$this->Cell(272,7,(($actype == "A330-300") || ($actype == "A330-200") ? "AIRBUS" : "BOEING")." ".$actype,'LRB',0,'C');
		$this->Ln();
...
$L = 14;
	...
	$this->Cell(60,4,(($actype == "A330-300") || ($actype == "A330-200") ? "AIRBUS" : "BOEING").' '.$actype,1,0,'C');
	$this->Cell($L,4,'',1,0,'C');
...
$this->_out($s);
  ...
  $bulanalert = substr($periode,-2,2);
	...
	$jumlahfleet = $fleet[$periode];//die(print_r($fleet));
	...
	foreach ($bulan as $k => $v){
		...
		$delayratelastyear = (array_sum($FCrevlastyear) <> 0) ? (array_sum($techdelaylastyear) / array_sum($FCrevlastyear) * 100) : 0;
	...
	$jumlahhal = ($jumlahfleet > 14) ? ($jumlahfleet-($jumlahfleet%14))/14 + 1 : 1;//die(print_r($fleet));
...
$panjang = $tinggi*54-$tinggi/2;
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi,(array_sum($FHlastyear) <> 0) ? round(array_sum($fleetlastyear)/12*$numberofengine,0) : 0,'TBLR',0,'C');
	/*for($i=1;$i<=12;$i++)
	{*/
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi,(array_sum($FHlastyear) <> 0) ? (array_sum($FHlastyear) * $numberofengine) : 0,'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi,(array_sum($FClastyear) <> 0) ? (array_sum($FClastyear) * $numberofengine) : 0,'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi,(array_sum($FClastyear) <> 0) ? $ifsdlastyear : 0,'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi,(array_sum($FHlastyear) <> 0) ? round(($ifsdlastyear/(array_sum($FHlastyear)*$numberofengine)*1000),3) : 0,'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
	...
	$pdf->Cell(60,$tinggi,'      - RATE','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, (array_sum($FHlastyear) <> 0) ? round(($rtolastyeardata/(array_sum($FHlastyear)*$numberofengine)*1000),3) : "",'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
	...
	$pdf->SetFont('arial','B',7);
	$pdf->Ln();
	$pdf->Cell(60,$tinggi,'CONVENIENCE','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi,$engineremovallastyear["Convenience"],'TBLR',0,'C');
	
	foreach($bulan as $key => $value)
	{
		
		$pdf->Cell($L,$tinggi,(round($engineremoval["Convenience"][$value])),'TBLR',0,'C');
	}
	//die(print_r($FH));
	$pdf->SetFont('arial','B',7);
	$pdf->Ln();
	$pdf->Cell(60,$tinggi,'CAMPAIGN','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi,$engineremovallastyear["Campaign"],'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi,$engineremoval["Campaign"][$value],'TBLR',0,'C');
	}
	//die(print_r($FH));
	$pdf->SetFont('arial','B',7);
	$pdf->Ln();
	$pdf->Cell(10,$tinggi,'','TLR',0,'L');
	$pdf->Cell(50,$tinggi,'PREDIFINE REMOVAL','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi,'','TBLR',0,'C');
	for($i=1;$i<=12;$i++)
	{
	...
	$pdf->Cell(50,$tinggi,'      * BASIC - PLANNED','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, $engineremovallastyear["Predefined Basic Planned"],'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi,$engineremoval["Predefined Basic Planned"][$value],'TBLR',0,'C');
	}
	//die(print_r($FH24));
	$pdf->SetFont('arial','B',7);
	$pdf->Ln();
	$pdf->Cell(10,$tinggi,'','LR',0,'L');
	$pdf->Cell(50,$tinggi,'                     - UNPLANNED','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi,$engineremovallastyear["Predefined Basic UnPlanned"],'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi,$engineremoval["Predefined Basic UnPlanned"][$value],'TBLR',0,'C');
	}

	$pdf->SetFont('arial','B',7);
	$pdf->Ln();
	$pdf->Cell(10,$tinggi,'','LR',0,'L');
	$pdf->Cell(50,$tinggi,'      * NON BASIC - PLANNED','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, $engineremovallastyear["Predefined NonBasic Planned"],'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi,$engineremoval["Predefined NonBasic Planned"][$value],'TBLR',0,'C');
	}
	//die(print_r($FH24));
	$pdf->SetFont('arial','B',7);
	$pdf->Ln();
	...
	$pdf->Cell(50,$tinggi,'                              - UNPLANNED','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi,$engineremovallastyear["Predefined NonBasic UnPlanned"],'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi, $engineremoval["Predefined NonBasic UnPlanned"][$value],'TBLR',0,'C');
	}

	$pdf->SetFont('arial','B',7);
	$pdf->Ln();
	$pdf->Cell(10,$tinggi,'','LR',0,'L');
	$pdf->Cell(50,$tinggi,'PREMATURE REMOVAL','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi,'','TBLR',0,'C');
	for($i=1;$i<=12;$i++)
	{
	...
	$pdf->Cell(50,$tinggi,'      * BASIC - PLANNED','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi,$engineremovallastyear["Premature Basic Planned"],'TBLR',0,'C');
		foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi,$engineremoval["Premature Basic Planned"][$value],'TBLR',0,'C');
	}
	//die(print_r($FH));
	$pdf->SetFont('arial','B',7);
	$pdf->Ln();
	...
	$pdf->Cell(50,$tinggi,'                     - UNPLANNED','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, $engineremovallastyear["Premature Basic UnPlanned"],'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi, $engineremoval["Premature Basic UnPlanned"][$value],'TBLR',0,'C');
	}
	...
	$pdf->Cell(50,$tinggi,'      * NON BASIC - PLANNED','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi,$engineremovallastyear["Premature NonBasic Planned"],'TBLR',0,'C');
		foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi,$engineremoval["Premature NonBasic Planned"][$value],'TBLR',0,'C');
	}
	//die(print_r($FH));
	$pdf->SetFont('arial','B',7);
	$pdf->Ln();
	...
	$pdf->Cell(50,$tinggi,'                              - UNPLANNED','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, $engineremovallastyear["Premature NonBasic UnPlanned"],'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi, $engineremoval["Premature NonBasic UnPlanned"][$value],'TBLR',0,'C');
	}
	//die(print_r($FH24));
...
$getRY = $pdf->GetY();
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, ($engineremovallastyear["Premature NonBasic UnPlanned"] + $engineremovallastyear["Premature Basic UnPlanned"] + $engineremovallastyear["Predefined Basic UnPlanned"] + $engineremovallastyear["Predefined NonBasic UnPlanned"]),'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi,  ($engineremoval["Predefined Basic UnPlanned"][$value] + $engineremoval["Predefined NonBasic UnPlanned"][$value] + $engineremoval["Premature Basic UnPlanned"][$value] + $engineremoval["Premature NonBasic UnPlanned"][$value]),'TBLR',0,'C');
	}
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, (array_sum($FHlastyear) <> 0) ? round(($engineremovallastyear["Premature NonBasic UnPlanned"] + $engineremovallastyear["Premature Basic UnPlanned"] + $engineremovallastyear["Predefined Basic UnPlanned"] + $engineremovallastyear["Predefined NonBasic UnPlanned"])/(array_sum($FHlastyear) * $numberofengine)*1000,3) : "",'TBLR',0,'C');
	//die(print_r($FH24));
foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi, (!empty($FH[$value]) ? round((($engineremoval["Predefined Basic UnPlanned"][$value] + $engineremoval["Predefined NonBasic UnPlanned"][$value] + $engineremoval["Premature Basic UnPlanned"][$value] + $engineremoval["Premature NonBasic UnPlanned"][$value])/($FH[$value] * $numberofengine)*1000),3) : 0),'TBLR',0,'C');

		$uer[] =  (!empty($FH[$value]) ? round((($engineremoval["Predefined Basic UnPlanned"][$value] + $engineremoval["Predefined NonBasic UnPlanned"][$value] + $engineremoval["Premature Basic UnPlanned"][$value] + $engineremoval["Premature NonBasic UnPlanned"][$value])/($FH[$value] * $numberofengine)*1000),3) : 0);
	}
	...
	$pdf->SetFont('arial','B',7);
	$pdf->Ln();
	$pdf->Cell(60,$tinggi,'TOTAL PREMATURE REMOVALS','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi,$engineremovallastyear["Premature Basic Planned"] + $engineremovallastyear["Premature Basic UnPlanned"] + $engineremovallastyear["Premature NonBasic Planned"] + $engineremovallastyear["Premature NonBasic UnPlanned"],'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi,($engineremoval["Premature Basic Planned"][$value] + $engineremoval["Premature Basic UnPlanned"][$value] + $engineremoval["Premature NonBasic Planned"][$value] + $engineremoval["Premature NonBasic UnPlanned"][$value]),'TBLR',0,'C');
	}
	...
	$pdf->Cell(60,$tinggi,'      - RATE','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, (array_sum($FHlastyear) <> 0) ? round((($engineremovallastyear["Premature Basic Planned"] + $engineremovallastyear["Premature Basic UnPlanned"] + $engineremovallastyear["Premature NonBasic Planned"] + $engineremovallastyear["Premature NonBasic UnPlanned"])/(array_sum($FHlastyear)*$numberofengine)*1000),3) : "",'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		$prematurerate[$value] = !empty($FH[$value]) ? round((($engineremoval["Premature Basic Planned"][$value] + $engineremoval["Premature Basic UnPlanned"][$value] + $engineremoval["Premature NonBasic Planned"][$value] + $engineremoval["Premature NonBasic UnPlanned"][$value])/($FH[$value] * $numberofengine)*1000),3) : 0;
		$pdf->Cell($L,$tinggi,$prematurerate[$value],'TBLR',0,'C');
		$premature[] = $prematurerate[$value];
	}
	...
	foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi, (!empty($FH24[$bulan12[$key+10]]) ? round((($engineremoval["Premature Basic Planned"][$bulan12[$key+10]] + $engineremoval["Premature Basic UnPlanned"][$bulan12[$key+10]] + $engineremoval["Premature NonBasic Planned"][$bulan12[$key+10]] + $engineremoval["Premature NonBasic UnPlanned"][$bulan12[$key+10]] + $engineremoval["Premature Basic Planned"][$bulan12[$key+11]] + $engineremoval["Premature Basic UnPlanned"][$bulan12[$key+11]] + $engineremoval["Premature NonBasic Planned"][$bulan12[$key+11]] + $engineremoval["Premature NonBasic UnPlanned"][$bulan12[$key+11]] + $engineremoval["Premature Basic Planned"][$value] + $engineremoval["Premature Basic UnPlanned"][$value] + $engineremoval["Premature NonBasic Planned"][$value] + $engineremoval["Premature NonBasic UnPlanned"][$value])/(($FH24[$bulan12[$key+10]] + $FH24[$bulan12[$key+11]] + $FH24[$bulan12[$key+12]]) * $numberofengine)*1000),3) : 0),'TBLR',0,'C');
	}
	...
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi,($engineremovallastyear["Premature Basic Planned"] + $engineremovallastyear["Premature Basic UnPlanned"] + $engineremovallastyear["Premature NonBasic Planned"] + $engineremovallastyear["Premature NonBasic UnPlanned"] + $engineremovallastyear["Predefined Basic Planned"] + $engineremovallastyear["Predefined Basic UnPlanned"] + $engineremovallastyear["Predefined NonBasic Planned"] + $engineremovallastyear["Predefined NonBasic UnPlanned"]),'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi, ($engineremoval["Premature Basic Planned"][$value] + $engineremoval["Premature Basic UnPlanned"][$value] + $engineremoval["Premature NonBasic Planned"][$value] + $engineremoval["Premature NonBasic UnPlanned"][$value] + $engineremoval["Predefined Basic Planned"][$value] + $engineremoval["Predefined Basic UnPlanned"][$value] + $engineremoval["Predefined NonBasic Planned"][$value] + $engineremoval["Predefined NonBasic UnPlanned"][$value] + $engineremoval["Campaign"][$value]),'TBLR',0,'C');
	}
	...
	$pdf->Cell(60,$tinggi,'      - RATE','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, (array_sum($FHlastyear) <> 0) ? round((($engineremovallastyear["Premature Basic Planned"] + $engineremovallastyear["Premature Basic UnPlanned"] + $engineremovallastyear["Premature NonBasic Planned"] + $engineremovallastyear["Premature NonBasic UnPlanned"] + $engineremovallastyear["Predefined Basic Planned"] + $engineremovallastyear["Predefined Basic UnPlanned"] + $engineremovallastyear["Predefined NonBasic Planned"] + $engineremovallastyear["Predefined NonBasic UnPlanned"])/(array_sum($FHlastyear)*$numberofengine)*1000),3) : "",'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		$shopvisitrate[$value] = !empty($FH[$value]) ? round((($engineremoval["Premature Basic Planned"][$value] + $engineremoval["Premature Basic UnPlanned"][$value] + $engineremoval["Premature NonBasic Planned"][$value] + $engineremoval["Premature NonBasic UnPlanned"][$value] + $engineremoval["Predefined Basic Planned"][$value] + $engineremoval["Predefined Basic UnPlanned"][$value] + $engineremoval["Predefined NonBasic Planned"][$value] + $engineremoval["Predefined NonBasic UnPlanned"][$value] + $engineremoval["Campaign"][$value])/($FH[$value] * $numberofengine)*1000),3) : 0;
		$pdf->Cell($L,$tinggi, $shopvisitrate[$value],'TBLR',0,'C');
		$shopvisit[] = $shopvisitrate[$value];
	}
	...
	foreach($bulan as $key => $value)
	{
		$pdf->Cell($L,$tinggi,($engineremoval["Premature Basic Planned"][$value] + $engineremoval["Premature Basic UnPlanned"][$value] + $engineremoval["Premature NonBasic Planned"][$value] + $engineremoval["Premature NonBasic UnPlanned"][$value] + $engineremoval["Predefined Basic Planned"][$value] + $engineremoval["Predefined Basic UnPlanned"][$value] + $engineremoval["Predefined NonBasic Planned"][$value] + $engineremoval["Predefined NonBasic UnPlanned"][$value] + $engineremoval["Campaign"][$value] + $engineremoval["Convenience"][$value]),'TBLR',0,'C');
	}

	$pdf->SetFont('arial','B',7);
	$pdf->Ln();
	$pdf->Cell(60,$tinggi,'      - RATE/100 ENG. HOURS','TBLR',0,'L');
	$pdf->SetFont('arial','',7);
	$pdf->Cell($L,$tinggi, (array_sum($FHlastyear) <> 0) ? round((($engineremovallastyear["Premature Basic Planned"] + $engineremovallastyear["Premature Basic UnPlanned"] + $engineremovallastyear["Premature NonBasic Planned"] + $engineremovallastyear["Premature NonBasic UnPlanned"] + $engineremovallastyear["Predefined Basic Planned"] + $engineremovallastyear["Predefined Basic UnPlanned"] + $engineremovallastyear["Predefined NonBasic Planned"] + $engineremovallastyear["Predefined NonBasic UnPlanned"] + $engineremovallastyear["Convenience"] + $engineremovallastyear["Campaign"])/(array_sum($FHlastyear)*$numberofengine)*1000),3) : "",'TBLR',0,'C');
	foreach($bulan as $key => $value)
	{
		$engremovalrate[$value] = !empty($FH[$value]) ? round((($engineremoval["Premature Basic Planned"][$value] + $engineremoval["Premature Basic UnPlanned"][$value] + $engineremoval["Premature NonBasic Planned"][$value] + $engineremoval["Premature NonBasic UnPlanned"][$value] + $engineremoval["Predefined Basic Planned"][$value] + $engineremoval["Predefined Basic UnPlanned"][$value] + $engineremoval["Predefined NonBasic Planned"][$value] + $engineremoval["Predefined NonBasic UnPlanned"][$value] + $engineremoval["Campaign"][$value] + $engineremoval["Convenience"][$value])/($FH[$value] * $numberofengine)*1000),3) : 0;
	...
	$rate = !empty($FHengremove) ? round((($convenience + $campaign + $predbasplan + $predbasunplan + $prednonbasplan + $prednonbaseunplan + $prembasplan + $prembasunplan + $premnonbasplan + $premnonbasunplan)/($FHengremove * $numberofengine) * 1000),3) : 0;
