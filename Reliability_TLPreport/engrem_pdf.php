...
//		$page = "E";
	...
	$this->Cell(124,4,'ENGINE SERIAL NUMBER','LRTB',2,'C');
	...
	$this->Cell(25,4,'BASIC','LRTB',0,'C');
	$this->Cell(25,4,'NON - BASIC','LRTB',1,'C');

	$this->writeRotie(23.5,40,'DATE',90);//TULISAN MIRING "DATE'
	$this->writeRotie(36,42,'STATION',90);//T'
	$this->writeRotie(49,43,'AIRCRAFT',90);
	$this->writeRotie(58.5,43,'POSITION',90);
	$this->writeRotie(67,48,'CONVINIENCE',90);
	$this->writeRotie(79,48,'CAMPAIGN',90);
	$this->SetX(84);
	$this->SetFont('Arial','',5.5);
	$this->Cell(12,6,'PLANNED','LRTB',0,'C');//tulisan tegak lagi
	...
	//$totalpage = '{nb}';
	//if ($totalpage <> '1'){
		$this->writeRotie(260,-15,'Page : '.$page."9 - ".$this->PageNo(),0);
	//} else {
	...
	$this->Line(15,23,279,23);
	$this->Line(15,49,279,49);
	
	$this->Line(15,23,15,49);
	$this->Line(279,23,279,49);
...
$qryFH = "SELECT `tbl_removaldesc`.`Reason`, `tbl_engineremove`.`DateRemove`, `tbl_engineremove`.`Station`, `tbl_engineremove`.`AircraftReg`, `tbl_engineremove`.`Pos`, `tbl_engineremove`.`SNOff`, `tbl_engineremove`.`TSI`,`tbl_engineremove`.`CSI`, `tbl_engineremove`.`TSN`, `tbl_engineremove`.`CSN`, `tbl_engineremove`.`Remark` FROM `tbl_engineremove` Inner Join `tbl_removaldesc` ON `tbl_engineremove`.`ReasonRemoval` = `tbl_removaldesc`.`ID` WHERE `tbl_engineremove`.`AircraftType` IN ('".$actype."') AND (DATE_FORMAT(`tbl_engineremove`.`DateRemove`,'%Y-%m') BETWEEN '".$lastperiode."' AND '".$_REQUEST["periode"]."') ORDER BY `tbl_engineremove`.`DateRemove` DESC";//die($qryFH);
	...
	while ($hasil = mysqli_fetch_array($resultFH)){
		...
		$pdf->Cell(12,$tinggi,$predbasicplan,'TBLR',0,'C');
		$pdf->Cell(13,$tinggi,$predbasicunplan,'TBLR',0,'C');
		$pdf->Cell(12,$tinggi,$prednonbasicplan,'TBLR',0,'C');
		$pdf->Cell(13,$tinggi,$prednonbasicunplan,'TBLR',0,'C');
