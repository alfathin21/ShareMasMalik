...
function selectactype() {
	$actypequery = "SELECT tbl_masterac.ACReg FROM tbl_masterac WHERE tbl_masterac.Active =  '1' ORDER BY ACReg ASC";//die($actypequery);
	...
	array_push($arractype,"B737-300C","B737-400C");
	return $arractype;
}
...
function export_excel()
  {
        if (document.frm1.periode.value == 0) {
        	   ...
        	   var str = "excel-reg.php?actype=" + escape(document.frm1.actype.value) + "&periode=" + escape(document.frm1.periode.value);
                window.open(str);
        }
  }
</script>
...
&nbsp;&nbsp;&nbsp;
  ...
  <!--<input type="submit" name="export" value="Export" onClick="export_excel()">--><a href="excel-reg.php?actype=<?= $actype?>&periode=<?= $periode?>" target="_blank">Export to Excel</a> 
  </p>
  <?php 
	...
	$arrFC = queryFC($weeknih,$actype);
	mysql_close($connectAMS);
 ...
 if ($week == "") {
  ...
  //die(print_r(weeks_in_year('2009')));
  ...
  <table width="558" border="1">
	...
	if (substr($periode,0,2) < 12){
		...
		for($i=0;$i<12;$i++)
		{	
			//$minggu[$i] = isset($weekkey[11-$i]) ? $weekkey[11-$i] : "-";
    ...
    <tr>
      <th scope="row"><div align="left">D &amp; C SUB </div></th>
      <? 
	  //$connectAMSdelay = connectimesysTE();
	  for($i=0;$i<12;$i++)
		{
